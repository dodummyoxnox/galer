import { useState, useEffect } from 'react';
import { request } from '../../utils/api';
import './Tentang.css';

export default function Tentang() {
  const [profile, setProfile] = useState(null);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    const loadTentang = async () => {
      try {
        const response = await request('/api/konten/tentang');
        if (response.success && response.data) {
          setProfile(response.data);
        }
      } catch (error) {
        console.error('Gagal memuat konten tentang:', error.message);
      } finally {
        setLoading(false);
      }
    };

    loadTentang();
  }, []);

  if (loading) {
    return (
      <div className="tentang-page section container" style={{ minHeight: '60vh', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <h2 className="font-mono" style={{ color: 'var(--color-muted)' }}>Memuat profil pelukis...</h2>
      </div>
    );
  }

  if (!profile) {
    return (
      <div className="tentang-page section container" style={{ minHeight: '60vh' }}>
        <h1 className="font-display">PROFIL TIDAK DITEMUKAN</h1>
        <p className="font-mono">Maaf, profil pelukis belum dikonfigurasi.</p>
      </div>
    );
  }

  return (
    <div className="tentang-page" id="tentang-page">
      {/* Hero */}
      <section className="tentang-hero section">
        <div className="container tentang-hero__inner">
          <div className="tentang-hero__photo">
            <img src={profile.foto} alt={profile.nama} />
            <div className="tentang-hero__photo-label font-mono">
              <span>EST. 2015</span>
              <span>YOGYAKARTA</span>
            </div>
          </div>

          <div className="tentang-hero__text">
            <h1 className="font-display tentang-hero__name">{profile.nama}</h1>
            <p className="tentang-hero__tagline font-heading">{profile.tagline}</p>
            <div className="divider-kasar" style={{ maxWidth: 80 }} />
            {profile.bio && profile.bio.split('\n\n').map((para, i) => (
              <p key={i} className="tentang-hero__bio font-mono">{para}</p>
            ))}
          </div>
        </div>
      </section>

      {/* Timeline */}
      {profile.timeline && profile.timeline.length > 0 && (
        <section className="section section--paper tentang-timeline" id="tentang-timeline">
          <div className="container">
            <h2 className="font-display" style={{ color: 'var(--color-ink-dark)', marginBottom: 'var(--space-3xl)' }}>
              PERJALANAN
            </h2>
            <div className="tentang-timeline__list">
              {profile.timeline.map((item, i) => (
                <div key={i} className="tentang-timeline__item">
                  <span className="tentang-timeline__year font-display">{item.tahun}</span>
                  <div className="tentang-timeline__dot" />
                  <p className="tentang-timeline__event font-mono">{item.peristiwa}</p>
                </div>
              ))}
            </div>
          </div>
        </section>
      )}
    </div>
  );
}
